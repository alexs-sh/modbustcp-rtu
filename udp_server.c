...
int udp_server_create()
{
    ...
    // создать udp сокет
    udp_server->fd  = socket_create(SOCKET_UDP); //socket_create_server(info->settings->ip, info->settings->port);
    result = socket_bind(udp_server->fd , info->settings->ip, info->settings->port);
    socket_set_blockmode(udp_server->fd , SOCKET_NON_BLOCK);

    if(!result ||
       udp_server->fd  == INVALID_FD)
        goto error;
...
static int udp_server_read(connection_t * conn) {
    ...
    if( (result = socket_read_udp_raw(conn->fd,ptr, MESSAGE_BUFFER_SIZE, &ip_addr, &port)) < MB_HEADER_SIZE) {
        return 0;
    }
    ...
    //printf("ID = %d PROTO = %d LEN = %d SIZE = %d\n", input_msg.pdu.id, input_msg.pdu.proto, input_msg.pdu.len, queue_get_size(&info->tty_info->input_queue));

    if( input_msg.pdu.proto != 0    ||
        input_msg.pdu.len == 0      ||
        ...
        //250 байт нагрузки + 7 заголовка (самый большой заголовок для функции записи 16)
...
static int udp_server_write(connection_t * conn, va_list args) {
    ...
    if(result)
        socket_write_udp_raw(message->src_connection->fd, out_buffer, result, message->src_ip, message->src_port);
