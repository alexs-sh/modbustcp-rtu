...
void cmd_server_create(common_info_t * info)
{
    ...
    //in->info = info;
    cmd_server->fd = socket_create_server(info->settings->ip, info->settings->cmd_srv_port);
...
static void print_info(common_info_t * info) {
    ...
    send_line_args(fd, buffer, CMD_CLIENT_BUFFER_SIZE, result, "Time:\t%s", time_buffer);

    send_line(fd, buffer, CMD_CLIENT_BUFFER_SIZE, result, "Server:");

    send_line_args(fd, buffer, CMD_CLIENT_BUFFER_SIZE, result,
                   "\tLocal IP:%s", info->settings->ip);
    send_line_args(fd, buffer, CMD_CLIENT_BUFFER_SIZE, result,
                   "\tLocal Port:%d", info->settings->port);
    send_line_args(fd, buffer, CMD_CLIENT_BUFFER_SIZE, result,
                   "\tMode:%s", info->settings->udp ? "UDP" : "TCP");
    ...
    if(!info->settings->udp) {
        ...
        send_line_args(fd, buffer, CMD_CLIENT_BUFFER_SIZE, result,
                        "\tDisconnect timeout, ms:%d", info->settings->disconnect_timeout_ms);
        send_line_args(fd, buffer, CMD_CLIENT_BUFFER_SIZE, result,
        ...
        for(i = 0, counter = 0; i < MAX_CONNECTIONS; i++) {
            ...
            #ifdef MX_BIG_ENDIAN
                send_line_args(fd, buffer, CMD_CLIENT_BUFFER_SIZE, result, "\t\t%d) %d.%d.%d.%d:%d",  counter,
    ...
    send_line_args(fd, buffer, CMD_CLIENT_BUFFER_SIZE, result,
                   "\tRX bytes:%.0f", info->tty_info->rx);
    send_line_args(fd, buffer, CMD_CLIENT_BUFFER_SIZE, result,
                   "\tTX bytes:%.0f", info->tty_info->tx);
...
void cmd_server_trace_out(int force, char * color, const char * format, ...)
{
    ...
    if(!(info->settings->net_trace || force))
            return;
...
void cmd_server_trace_buffer_out(char * color, uint8_t * buffer, uint16_t size)
{
    ...
    if(!info->settings->net_trace)
            return;

    // запретить использовать слишком большой размер выходных данных
    if(size > CMD_CLIENT_BUFFER_SIZE * 4)
        size = CMD_CLIENT_BUFFER_SIZE * 4;
    ...
    for(i = 0; i < size && buff_ptr < buff_end - 1; i++) {
        ...
        result = snprintf(buff_ptr, CMD_CLIENT_BUFFER_SIZE * 4 - buff_pos - 2, "%d ", buffer[i]);
