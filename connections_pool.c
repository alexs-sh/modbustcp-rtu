...
connections_pool_t* connections_pool_create(uint8_t nconn, uint16_t info_size) {
  ...
  for (i = 0; i < nconn; i++) {
    //выделение одной области для коннекта и данных. Область данных идет сразу
    //после инормации о коннекте  connection_t * new_connect = calloc(1,
    // sizeof(*new_connect) + info_size);
    connection_t* new_connect = calloc(1, sizeof(*new_connect) + info_size);
    if (!new_connect) goto error;
...
void connections_pool_free(connections_pool_t* pool) {
  ...
  next_conn = pool->busy;
  while (next_conn) {
    if (next_conn->close_func) connection_close(next_conn);
    next_conn = next_conn->next;
  }
 ...
 * @param pool - указатель на пул
 * @return Возвращает указатель на свободный коннект. Если объектов нет,
 * возвращает 0
 */
...
int connections_pool_release_obj(connections_pool_t* pool, connection_t* obj) {
  ...
  if (conn != obj) return 0;

  // если есть функция закрытия, то сначала вызвать её
  if (conn->close_func) connection_close(conn);
...
/** @brief Функция, информирующая пул, что данный оннект больше не нужен и может
 * юыть закрыт
 * @param pool - указатель на пул
 * @param obj - указатель на освобождаемый коннект
 */
void connections_pool_invalidate_obj(connections_pool_t* pool,
                                     connection_t* obj) {
  assert(pool);
  assert(obj);
...
/** @brief Функция для перемещения файловых дескрипторов коннектов в структуру
 * fd_set. Необходима для работы с такими системынми функциями, как select и
 * poll.
 * @param pool - указатель на пул
 * @param set - указатель на принимающую структуру fd_set
 * @param max_fd - указатель на переменную, которая будет хранить значения
 * максимально большого дескриптора
 * @return Возвращает 1 в случае успеха
 */
void connections_pool_to_fdset(connections_pool_t* pool, fd_set* set,
                               int* max_fd) {
  connection_t* conn = 0;
  ...
  while (conn) {
    if (conn->fd != INVALID_FD) {
      ...
      if (*max_fd < conn->fd) *max_fd = conn->fd;
    }
 ...
 * @param pool - указатель на пул
 * @param timeout_ms - таймаут в миллисекундах на ожидание хотя-бы одноо
 * активного соединения
 * @return В случае успеха возвращает кол-во миллимекунд, потраченных на
 * ожидание активности
 */
uint32_t connections_pool_process(connections_pool_t* pool,
                                  uint32_t timeout_ms) {
  fd_set set;
  ...
  // есть активные коннекты
    ...
    while (conn && result) {
      ...
      if (FD_ISSET(act_conn->fd, &set) && act_conn->read_func) {
