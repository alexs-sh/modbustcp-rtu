 ...
 * @return Возвращает константу, соответсвующую скорости. Либо константу, соотв. скорости 9600 Бит/с
 *
 */
 ...
 *
 */
int tty_setup(int fd, uint32_t speed, uint8_t parity, uint8_t data_bits, uint8_t flow, uint8_t block, uint8_t stop_bits)
{
...
void tty_tests()
{
    ...
    if(!tty_setup(port, 38400, TTY_PARITY_ODD, 8, TTY_FLOW_NONE, TTY_NONBLOCK_MODE, 1))
    {
