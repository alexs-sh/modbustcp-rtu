...
#define time_to_mks(t)   ((t).tv_sec * 1000000.0 + (t).tv_usec)     ///< Перевод времени из struct timveal в микросекунды

#define mks_to_time(dt)   (struct timeval){(long)(dt) / 1000000,  (dt) - ((long)(dt) / 1000000) * 1000000}  ///< Перевод времени из struct timveal в микросекунды
...
struct settings_t
{
    ...
    uint32_t silence_timeout_ms;     ///< таймаут на сброс сообщения на посл. устройстве

    uint32_t disconnect_timeout_ms;  ///< таймаут на отключение неактивных клиентов
    ...
    uint8_t err_code_timeout;       ///< код ошибки передаваемый на ВУ в случае превышения времени ожидания ответа. По умолчанию 11.

    uint8_t err_code_overflow;      ///< код ошибки, выдваемый ри переполнении очереди сообщений. По умолчанию 11.

    uint8_t err_code_illegal;       ///< код ошибка, выдаваемый при получении некорректных данных по посл. порту. по умолчанию 3.
...
struct common_info_t
{
    ...
    struct connection_t * cmd_srv;                  ///< указатель на приемник управляющий подключений
    ...
    //struct tcp_server_info_t * tcp_server_info;             ///< указатель на структуру с данными о сервере и клиентах

    struct low_client_info_t * tty_info;            ///< указатель на структуру с данными о посл. порте
