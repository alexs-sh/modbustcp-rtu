...
//#define connection_get_info(conn)      (conn)->info    /**< Получить указатель на область данных */

//#define connection_next(conn)          (conn)->next    /**< Получить указатель на сл. элемент */

//#define connection_to_fd(conn)         (conn)->fd      /**< Получить файловый дескриптор */
...
/**@struct connection_t
* Элемент списка подключений. Элементы могут объединятся в односвязный список. Кажый элемент
* содержит связку из файлового дескриптора и данных, необходимых для обработки дескриптора.
*/
struct connection_t
{
