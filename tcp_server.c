...
static void tcp_server_update_client_info(tcp_server_info_t* tcp_server,
                                          connection_t* client);
...
int tcp_server_accept(connection_t* conn) {
  ...
  connect->fd = client;
  connect->read_func =
      (conn_handler_t)tcp_server_read_client;  // high_client_read;
  connect->close_func =
      (conn_handler_t)tcp_server_close_client;  // high_client_close;
...
int tcp_server_close(connection_t* conn) {
  ...
  if (server_info->clients_info) free(server_info->clients_info);
...
error:
  ...
  if (server_info && server_info->clients_info) free(server_info->clients_info);

  if (server_info) free(server_info);
...
void tcp_server_check_clients(tcp_server_info_t* tcp_server_info, int size,
  ...
  for (i = 0; i < size; i++) {
    if (!tcp_server_info->clients_info[i].client) continue;

    if (now_float - time_to_mks(tcp_server_info->clients_info[i].last_update) >=
        1000.0 * timeout_ms)
      connections_pool_invalidate_obj(
          tcp_server_info->clients_info[i].client->pool,
          tcp_server_info->clients_info[i].client);
  }
}
