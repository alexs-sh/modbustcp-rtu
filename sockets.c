 ...
 *
 * @param s - дескриптор.
 * @param sock_addr - адрес интерфейса, к которому будет привязан сокет. Может быть 0 (без привязки к конткретному интерфейсу), либо содержать ip - адрес интерфейса, например 127.0.0.1.
 ...
 *
 * @param s - дескриптор.
 * @param client_addr - указатель на структура, которая будет заполнена данными о подключаемой машине (ip, port). Может быть 0.
 ...
 * При вызове блокирует выполнение, пока не будет выполнено подключение, либо время ожидания не превысит таймаут.
 * @param s - дескриптор.
...
/** @brief Функция для создания TCP сервера, связанного с заданным интефейсом и портом.
 *
...
int socket_create_server(const char * srv_addr, uint16_t port)
{
    ...
    if(!socket_bind(sock, srv_addr, port))
        return SOCKET_INVALID;
...
int socket_set_timeouts(int s, uint32_t read_timeout_ms, uint32_t write_timeout_ms)
{
    ...
    if(read_timeout_ms)
    {
        ...
        r_result = (setsockopt (s, SOL_SOCKET, SO_RCVTIMEO, (char *)&timeout, sizeof(timeout)) == 0);
    }
    ...
    if(write_timeout_ms)
    {
        ...
        w_result = (setsockopt (s, SOL_SOCKET, SO_SNDTIMEO, (char *)&timeout, sizeof(timeout)) == 0);
    }

    return r_result && w_result;
}

/** @brief Функция для асинхронного подключения к удаленной машине. Дополнительно реализует набор проверок
 * на специфические ошибки, возникающие при асинхронном подключении.
 *
 * @param s - дескриптор.
 * @param srv_addr - ip - адрес интерфейса. Может быть 0.
 * @param port - порт сервера.
 * @return Возвращает 1 в случае успеха или необходимости подождать результат подключения. Если связь с удаленной
 * машиной однозначено невозможна, возвращает 0.
 *
 */
...
int socket_connect_async(int s, const char * srv_addr, uint16_t srv_port)
{
    ...
    if(!socket_set_blockmode(s, SOCKET_NON_BLOCK))
        return 0;
    ...
    if(result < 0)
    {
        /* Подкючение в процессе выполнения - все ОК. Во всех остальных случаях - ошибка.*/
 ...
 *
 * @param s - дескриптор.
 * @return Возвращает 2 - байтовое число содержащее подключенный порт на удаленной машине.
 *
 */
 ...
 * @param dst - указатель на буфер - приемник
 * @param size - размер буфера
 * @param ip_addr - переменная, которая будет хранить ip - адрес источника данных. Может быть 0.
 * @param port - переменная, которая будет хранить порт источника данных. Может быть 0.
 * @return Возвращает кол - во прочитанных байт. В случае ошибки возвращает -1.
 */
