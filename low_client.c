...
static void low_client_set_status(low_client_info_t *client_info,
                                  tty_status status);
...
int low_client_create() {
  ...
  // tty_conn->fd);
  if (tty_conn->fd == INVALID_FD) goto error;
  // выполнить настройки порта
  if (!tty_setup(tty_conn->fd, info->settings->speed, info->settings->parity,
                 info->settings->data_bits, TTY_FLOW_NONE, TTY_NONBLOCK_MODE,
                 info->settings->stop_bits))
    goto error;

  //  связать файловый дескриптор с функциями - обработчиками
  tty_conn->read_func = (conn_handler_t)low_client_read;
  tty_conn->write_func =
      (conn_handler_t)low_client_write;  // low_client_send_to_port;
...
error:
  if (info->tty_info) free(info->tty_info);

  if (tty_conn) connections_pool_release_obj(info->connections_pool, tty_conn);
  ...
  if (queue_overflow(&tty_info->input_queue)) {
    ...
    if (message_validate_source(message)) {
      if (info->settings->err_code_overflow) {
        message_make_error(
            message, info->settings->err_code_overflow);  // NO_RESPOND_ERROR);
...
void low_client_check() {
  ...
  if (low_client_is_clear(tty_info)) return;
  ...
  if (now - last > limit) {
    ...
    // trace_buffer_out(ANSI_COLOR_RED, tty_info->current_message.pdu.data,
    // tty_info->current_message.pdu.len);

    // если код ошибки задан для данной ситуации, то уведомить клиента
    if (info->settings->err_code_timeout) {
      message_make_error(&tty_info->current_message,
                         info->settings->err_code_timeout);
      connection_write(tty_info->current_message.src_connection,
                       &tty_info->current_message);
    }
...
void low_client_invalidate_conn(low_client_info_t *tty,
...
static int low_client_read(connection_t *conn) {
  ...
  result = tty_read(tty_info->tty->fd, ptr,
                    MESSAGE_BUFFER_SIZE - current_message->pdu.len);
  ...
  if (current_message->pdu.len > MAX_MESSAGE_SIZE ||
    ...
    trace_buffer_out(ANSI_COLOR_RED, current_message->pdu.data,
                     current_message->pdu.len);
    ...
    /* TODO (alexs#1#): Сделать таймаут, задаваемый через параметры ! */ wait =
        (struct timespec){0, 5 * 1000 * 1000};
    nanosleep(&wait, 0);
  ...
  if (calc_crc16(current_message->pdu.data, current_message->pdu.len) == 0) {
    ...
    trace_buffer_out(ANSI_COLOR_YELLOW, current_message->pdu.data,
                     current_message->pdu.len + 2);
...
static int low_client_send_to_port(connection_t *conn) {
  ...
  while (queue_pop(&tty_info->input_queue, current_message)) {
    ...
    if (!current_message->src_connection) continue;

    // Если контроль времени жизни сообщения отключен, то любое сообщение из
    // очереди счиатается корректным и обрабатывается.
    ...
    if (now - last < limit) {
      ...
      /* TODO (alexs#9#): Проверить есть ли необходимость уведомлять клиентов об
       * устаревании сообщений в очереди */
  ...
  result = tty_write(info->tty_info->tty->fd, ptr,
                     current_message->pdu.len + sizeof(crc));
  ...
  if (result <= 0) {
    ...
    trace_buffer_out(ANSI_COLOR_GREEN, current_message->pdu.data,
                     current_message->pdu.len + sizeof(crc));
  }
