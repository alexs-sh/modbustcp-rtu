...
settings_t current_settings = { 0,
                                ...
                                0,
                                MB_NO_RESPOND_ERROR,
                                MB_SLAVE_BUSY_ERRROR,
                                MB_ILLEGAL_DATA_ERROR,
                                0,
                                0
                              };
...
void print_help()
{
    ...
    printf("Modbus TCP / RTU driver\n"

           "\t-b                Run driver in background mode. Disabled for uLinux devices (UC - 7110)\n"
           "\t-v                View-mode. Enable input and output. Support commands 'quit' and 'info'\n"
           "\t-U                UDP.\n"
           "\t-I <name>         TCP server. Linked IP address (\"0.0.0.0\" by default)\n"
           ...
           "\t-t <value>        Seral port. Message drop timeout (50 ms by default)\n"
           "\t-d <value>        TCP server. Client disconnect timeout (30 000 ms by default)\n"
           "\t-w <value>        Queue. Message lifetime (1000 ms by default)\n"
           "\t-u <value>        Message.Error code for unresponsive device (11 by default)\n"
           "\t-o <value>        Message.Error code for queue overflow (6 by default)\n"
           "\t-i <value>        Message.Error code for illegal message data (3 by default)\n"
...
int parse_settings(settings_t * settings, int argc, char *argv[])
{
    ...
    while((c = getopt(argc, argv, opt_string)) != -1)
    {
        ...
        /*case 'q':
            ...
            break;*/
            /* вывод справки */
                      /* "\t-r <value>        Message.Error code for unresponsive device (11 by default)\n"
           "\t-o <value>        Message.Error code for queue overflow (6 by default)\n"
           "\t-i <value>        Message.Error code for illegal message data (3 by default)\n"*/
...
static int init_common_info(common_info_t * info)
{
    ...
    info->connections_pool = connections_pool_create(MAX_CONNECTIONS + 5, sizeof(void*));

    if(!info->connections_pool)
        return 0;
...
int main(int argc, char *argv[])
{
    ...
    if(!parse_settings(&current_settings, argc, argv))
        return 0;
    ...
    while(info.work)
    {
        ...
        uint32_t result = connections_pool_process(info.connections_pool,
                                                   POLL_TIMEOUT_MS);
